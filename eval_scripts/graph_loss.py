...
def main():
    ...
    for (path, label), color in zip(MODELS, COLORS):
        ...
        # -- fit line (power law) --
        mask = tokens > 0
        log_t, log_l = np.log(tokens[mask]), np.log(loss[mask])
        coeffs = np.polyfit(log_t, log_l, 1)
        fit_tokens = np.linspace(tokens[mask].min(), tokens[mask].max(), 200)
        ...
        ax.plot(fit_tokens, fit_loss, "-", color=color, linewidth=2.5,
                label=f"{label} ({loss_100M:.3f} @100M)")

    ax.set(xlabel="Tokens (M)", ylabel="Train Loss",
           title="100M Training Curves")
    ax.set_ylim(0.8, 4.0)
