...
Our key finding is stark: replacing Mamba with a 4-token convolution matches performance (1.79 vs 1.82 nats) at a fraction of the complexity, and a model with \emph{no} sequence mixing at all (pure linear projection + Hebbian) still converges to 3.18 nats. 
...
We evaluate both models using their recurrent (autoregressive) form on 4096-token windows from The Stack validation set, averaging over 8 windows. Table~\ref{tab:recurrent} shows per-segment losses.
...
Conv+Heb & 1.46 & 1.35 & 1.47 & 1.44 & \textbf{1.43} \\
\bottomrule
\end{tabular}
\end{table}

Conv+Heb maintains a 0.54-nat advantage across all positions, confirming that the training-time advantage transfers to autoregressive generation. Both models show stable loss across sequence length---neither degrades at longer positions.
