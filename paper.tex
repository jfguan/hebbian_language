...
The results are larger than expected. An 18M-parameter memory model reaches a validation loss of 1.848 nats on Python code versus 2.787 for a parameter-matched baseline ($+$0.939 nats), surpassing the baseline's final performance by step 200 of 1465. Scaling to 100M parameters, the memory model consistently leads the 100M baseline throughout training. The gap grows with context length as $W$ accumulates associations at test time. On prose the effect is smaller but consistent (2.477 vs.\ 2.527 for the deep baseline), suggesting the mechanism is broadly useful and especially powerful when the data is highly associative.
